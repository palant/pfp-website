...
Object.defineProperty(exports, "state", {
  ...
  get: function get() {
    ...
    return storage.has(hmacSecretKey).then(function (value) {
      if (value) return "set"; // Try legacy format
...
function clearData(noLock) {
  var _require = __webpack_require__(/*! ./passwords */ 1),
      lock = _require.lock;
...
function importPasswordData(data, masterPass) {
  var importers = [__webpack_require__(/*! ./importers/default */ 20), __webpack_require__(/*! ./importers/legacy */ 10), __webpack_require__(/*! ./importers/lastPass */ 21)];
...
var _require = __webpack_require__(/*! ./eventTarget */ 13),
...
module.exports = {
  ...
  runtime: {
    getURL: function getURL(path) {
      if (path == "worker/scrypt.js") return textToURL(__webpack_require__(/*! ../worker/scrypt.js */ 14));else if (path == "worker/pbkdf2.js") return textToURL(__webpack_require__(/*! ../worker/pbkdf2.js */ 15));else return path;
    },
...
var _require = __webpack_require__(/*! ./typedArrayConversion */ 16),
...
function getLink(_ref) {
  var type = _ref.type,
      param = _ref.param;
  if (type == "relnotes") return "https://pfp.works/release-notes/".concat(param);else if (type == "documentation") return "https://pfp.works/documentation/".concat(param, "/");
...
function getProvider(provider) {
  if (provider == "dropbox") return __webpack_require__(/*! ./sync-providers/dropbox */ 17);else if (provider == "gdrive") return __webpack_require__(/*! ./sync-providers/gdrive */ 18);else throw new Error("Unknown sync provider");
}
...
var engine = {
  ...
  sync: function sync() {
    ...
    var action = (nestingLevel ? Promise.resolve() : lock.acquire()).then(function () {
      ...
      if (!remote || _typeof(remote) != "object" || remote.application != local.application || remote.format != local.format || !remote.data || _typeof(remote.data) != "object" || typeof remote.data[masterPassword.saltKey] != "string" || typeof remote.data[masterPassword.hmacSecretKey] != "string") {
        throw "sync_unknown_data_format";
      }
...
function triggerModificationListeners() {
  for (var _i4 = 0, _modificationListener = modificationListeners; _i4 < _modificationListener.length; _i4++) {
    var listener = _modificationListener[_i4];
    listener();
  }
}
...
function import_(data, setRaw, setSite, setPassword) {
  return Promise.resolve().then(function () {
    ...
    if (!data || _typeof(data) != "object" || data.application != "easypasswords" || data.format != 1 || !data.sites || _typeof(data.sites) != "object") {
      throw "unknown_data_format";
    }
    ...
    for (var site in data.sites) {
      ...
      if (!importedData || _typeof(importedData) != "object") continue;
...
function import_(data, setRaw, setSite, setPassword, masterPass) {
  return Promise.resolve().then(function () {
    ...
    if (!data || _typeof(data) != "object" || data.application != "pfp" || data.format != 2 || !data.data || _typeof(data.data) != "object" || typeof data.data[masterPassword.saltKey] != "string" || typeof data.data[masterPassword.hmacSecretKey] != "string") {
      throw "unknown_data_format";
    }
...
var importer = __webpack_require__(/*! ./importers/legacy */ 10);
